...
class FastPlateDetector:
    def __init__(self):
        ...
        # ШАЛГУУР - зөөлөн болгосон
        self.MIN_CONFIDENCE = 50  # 50% (өмнө 65)
    ...
    def enhance_plate_fast(self, plate_img):
        ...
        return [binary1, binary2, binary3, gray]
    ...
    def ocr_robust(self, images):
        ...
        whitelist = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ'
...
def main():
    ...
    # Display size - 1280x720 тогтмол
    display_w = 1280
    display_h = 720
    
    print("🚀 Эхэлж байна...\n")
    print("   SPACE - Зогсоох/Үргэлжлүүлэх")
    print("   F     - Full screen on/off")
    print("   Q     - Дуусгах\n")
    print("-"*70 + "\n")
    ...
    while True:
        if not paused:
            ...
            # 5 frame тутамд таних (өмнө 8 байсан - одоо илүү олон таних)
            if frame_count % 5 == 0:
                ...
                for (x, y, w, h) in plates:
                    ...
                    # Шалгах: ЗӨӨЛӨН шалгуур
                    if text and conf >= detector.MIN_CONFIDENCE:  # 50%+
        ...
        # Full screen харуулах
        cv2.namedWindow('Video Plate Detection - Final', cv2.WINDOW_NORMAL)
        cv2.setWindowProperty('Video Plate Detection - Final', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
        ...
        if key == ord('q') or key == ord('Q'):
            ...
            # F товч - Full screen toggle
            prop = cv2.getWindowProperty('Video Plate Detection - Final', cv2.WND_PROP_FULLSCREEN)
            if prop == cv2.WINDOW_FULLSCREEN:
                cv2.setWindowProperty('Video Plate Detection - Final', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_NORMAL)
                print("🖼️  Normal харагдалт")
            else:
                cv2.setWindowProperty('Video Plate Detection - Final', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
                print("🖥️  Full screen")
