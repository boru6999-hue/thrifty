...
class FastPlateDetector:
    ...
    def ocr_improved(self, img):
        ...
        Энэ функц нь `mon` (Mongolian) эсвэл `rus` (Russian) traineddata-г ашиглана.
        Орос хэл нь Монгол кирилл үсгийг танина.
        """
    ...
    def clean_and_fix_text(self, text):
        ...
        Функц нь зөвхөн Монгол кирилл том үсэг (А..Я, Ө, Ү, Ё) ба цифрүүдийг
        үлдээж, бусад бүх тэмдэгтийг устгана. Мөн формат руу засна:
        - Эхний 4 нь тоо байх ёстой
        - Сүүлийн 3 нь үсэг байх ёстой
        - Нийт 7 тэмдэгт (6-8 тэмдэгт байвал засах)
        """
        if not text:
            return None
    ...
    def save_result(self, plate_img, text, video_time):
        ...
        try:
            ...
            if success:
                # Файлын замыг хадгалах (GUI дээр дарахад нээхэд ашиглана)
...
def main():
                        ...
                        if not detector.is_duplicate(text, frame_count, video_time):
                            ...
                            if most_count > detector.most_detected_count:
                                ...
                                if old_most_plate != most_plate:
                                    # Шинэ хамгийн олон удаа танигдсан дугаарыг хадгалах
                                    if text == most_plate:
                                        # Одоогийн дугаар нь хамгийн олон удаа танигдсан бол хадгалах
                                        detector.save_result(plate_img, text, video_time)
                                    
                                    # Хуучин хамгийн олон удаа танигдсан дугаарын файлуудыг устгах
                                    if old_most_plate and old_most_plate in detector.plate_to_file:
                                        old_file = detector.plate_to_file[old_most_plate]
                                        try:
                                            if os.path.exists(old_file):
                                                os.remove(old_file)
                                                print(f"🗑️  Хуучин дугаарын файл устгасан: {os.path.basename(old_file)}")
    ...
    if os.path.exists(save_folder_path):
        saved_files = [f for f in os.listdir(save_folder_path) if f.endswith('.jpg')]
        print(f"\n💾 Хадгалагдсан файлууд: {len(saved_files)}")
        ...
        if saved_files and len(saved_files) <= 10:
            ...
            print(f"   (Эхний 5 файл):")
            for f in saved_files[:5]:
                print(f"     - {f}")
            print(f"     ... ба {len(saved_files) - 5} файл")
    else:
